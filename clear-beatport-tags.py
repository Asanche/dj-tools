...
def clear_genre_if_capitalized_or_special(folder_path):
...
def main():
    parser = argparse.ArgumentParser(description='Clear genre tags that start with a capital letter, number, or symbol.')
    parser.add_argument('-path', type=str, help='Relative path to the folder', required=True)
    args = parser.parse_args()

    folder_path = args.path
    clear_genre_if_capitalized_or_special(folder_path)
